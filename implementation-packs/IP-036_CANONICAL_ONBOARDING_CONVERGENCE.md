...
legacy deletion for A-scope current-state staffing readers only.

Post-Batch-A redesign note (2026-08-05): owner feedback further freezes that
legacy skills import/data are not operationally usable and should be retired,
not migrated into onboarding. The follow-on plan below therefore narrows the
retained convergence scope to:

- **B1** — retire legacy skills feature dependence and delete the unsupported
  sidecar;
- **B2-B4** — converge HIREF / contract coverage as the remaining retained data
  family;
...
| **B0 — post-Batch-A redesign freeze** | Freeze the owner-approved redesign that retires legacy skills instead of migrating it, and decompose the retained contract-coverage follow-on work into bounded slices | `architecture/16_CANONICAL_ONBOARDING_CONVERGENCE_DESIGN.md`; this implementation pack; `PROGRESS.md` only | Read-only document review; `git diff --check` | B1-B4 boundaries, deletion targets, and deferred future-skills rule are explicit and stable; no runtime behavior changes were made. **This slice is now complete locally as a docs-only redesign update.** | Runtime, schema, or test behavior changes; reopening Batch A; inventing a new skills capability |
...
| `test_staffing_pipeline.py` skill match, `import-skills-matrix` blockers, staffing CLI `--skills`, and related scoring paths | These are the explicit runtime seams that still make the unsupported skills sidecar behave like a product feature | **B1** |
| `test_database_path_resolution.py`, `src/scripts/import_skills.py`, and docs that still advertise skills import | These prove whether the legacy script is still treated as a supported operator path | **B1** |
...
| `staffing` HIREF context, contract freshness blockers, and Dashboard contract/member/project context | These are the named read surfaces that must migrate once contract coverage has its own publication/read contract | **B3/B4** |
...
## Validation plan for docs-only redesign

This redesign update remains documentation-only. Validation stays
