...
`IP-000` and `IP-001` currently have complete implementation-pack documents.
Other items remain queued architecture work and must be expanded from the
standard template before implementation.
...
| IP-001 | Unified Use Case Execution Contract | IP-000 | G1 | READY FOR LOCAL ASSESSMENT |
| IP-002 | Structured Copilot Tool Transport | IP-001 | G2 | QUEUED |
| IP-003 | Evidence, Freshness, and Execution Trace Envelope | IP-001 | G2 | QUEUED |
...
4. Author IP-002 and IP-003 only after the shared executor shape is proven.
5. Do not start the Staffing write path until G2 has passed.
