...
SELECT
    ...
    m.employment_status AS employee_status,
    COALESCE(
        (
            SELECT e.max_parallel
            FROM employees e
            WHERE e.id = m.member_id OR COALESCE(e.wd_id, '') = m.member_id
            LIMIT 1
        ),
...
SELECT
    ...
    (
        SELECT legacy.jira_key
        FROM projects legacy
        WHERE legacy.id = p.project_id
        LIMIT 1
    ) AS jira_key,
    (
        SELECT legacy.target_end
        FROM projects legacy
        WHERE legacy.id = p.project_id
        LIMIT 1
    ) AS target_end,
...
JOIN current_state_staffing_members m
  ON m.publication_id = a.publication_id
 AND m.member_id = a.member_id
ORDER BY p.priority, p.display_name, m.display_name;
"""
