...
from pm_agent.data_onboarding import aux_file_source
...
def _aux_file_handler(
    definition: aux_file_source.AuxiliaryFileSourceDefinition,
) -> SourceTypeHandler:
    return SourceTypeHandler(
        source_type=definition.source_type,
        validate_profile=lambda profile: aux_file_source.validate_profile(
            definition, profile
        ),
        profile_metadata=lambda profile: aux_file_source.profile_metadata(
            definition, profile
        ),
        build_source_identity=lambda profile: aux_file_source.build_source_identity(
            definition, profile
        ),
        preview=lambda profile, *, run_id, db_path=None: aux_file_source.preview(
            definition,
            profile,
            run_id=run_id,
            db_path=db_path,
        ),
        confirm=lambda profile, source_preview, *, db_path=None: aux_file_source.confirm(
            definition,
            profile,
            source_preview,
            db_path=db_path,
        ),
        planned_operations=lambda source_preview: aux_file_source.planned_operations(
            definition,
            source_preview,
        ),
        coverage_summary=lambda source_preview: aux_file_source.coverage_summary(
            definition,
            source_preview,
        ),
        publication_links=lambda source_result: aux_file_source.publication_links(
            definition,
            source_result,
        ),
        release_run=lambda run_id, *, db_path=None: aux_file_source.release_run(
            definition,
            run_id,
            db_path=db_path,
        ),
...
_REGISTRY = {
    ...
    aux_file_source.PROJECT_PROFILE_WORKBOOK_SOURCE_TYPE: _aux_file_handler(
        aux_file_source.PROJECT_PROFILE_WORKBOOK_DEFINITION
    ),
    aux_file_source.SERVICENOW_CHANGE_REQUEST_CSV_SOURCE_TYPE: _aux_file_handler(
        aux_file_source.SERVICENOW_CHANGE_REQUEST_CSV_DEFINITION
    ),
}
