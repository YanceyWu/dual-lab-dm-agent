...
`src/pm_agent/dashboard/server.py`; focused continuity updates in
`src/tests/test_current_state_staffing.py`,
`src/tests/test_unified_use_case_contract.py`,
`src/tests/test_data_integrity_concurrency.py`,
`tools/rehearse_release.py`, `tools/tests/test_rehearse_release.py`,
`docs/REAL_ENVIRONMENT_UAT_RUNBOOK.md`, and `PROGRESS.md`. D2 did not delete
`/api/hiref` legacy-result-projection status or the dormant
`import-resource-portal` / `import-hiref-report` source rows because retained
B4 slot/result behavior and historical sync-run audit continuity still depend
on them. No D3, consumer redesign, dormant-source-row deletion, legacy-read
cleanup beyond the named artifacts, runtime redesign, connector redesign, or
mutable staffing-write redesign is present. The
...
IP-036 Batch D is now owner-accepted at the local repository baseline. The
accepted Batch D result builds on the clean submitted C3 baseline commit
`695c8ec9d2153ceb2a684200ef647522ef1249e9`, the required clean local C4 commit
`0f6108eddba77b0675573c02ef1b0559eb0ddfb9`, the committed D1 slice
`a3eea2762e29ee5494b7f8b5b278b8750f9ec6a7`, and the committed D2 slice
`f1ed9557f99d054480cb379fe2f1d41b38504049`.

Validation / review evidence for the accepted Batch D baseline:

- `src/.venv/bin/python -m pytest src/tests/test_current_state_staffing.py src/tests/test_unified_use_case_contract.py src/tests/test_data_integrity_concurrency.py tools/tests/test_rehearse_release.py -q` (`44 passed`)
- `src/.venv/bin/python -m ruff check src/pm_agent/dashboard/server.py src/pm_agent/current_state_staffing/schema.py src/tests/test_current_state_staffing.py src/tests/test_data_integrity_concurrency.py src/tests/test_unified_use_case_contract.py tools/rehearse_release.py tools/tests/test_rehearse_release.py`
- `git diff --check`
- independent read-only self-review: `D2 independent review passed`

The accepted Batch D baseline preserves the frozen boundaries:

- only the stale project-snapshot legacy marker and the now-unneeded
  current-state compatibility views are removed;
- release-rehearsal / UAT continuity checks now point to canonical
  `current_state_staffing_*` publication tables rather than deleted views;
- `/api/hiref` remains an explicit `legacy-result-projection` surface because
  B4 retained HIREF slot/result behavior still survives there;
- dormant `import-resource-portal` / `import-hiref-report` source rows remain in
  place because historical sync-run audit continuity still depends on them;
- no D3 slice is currently frozen or authorized; consumer redesign, broader
  bootstrap cleanup, runtime redesign, connector redesign, skills redesign, and
  mutable staffing-write redesign remain out of scope.
...
### 2026-08-08 — IP-038 Slice 4 spec review loop passed and is awaiting owner review

- Iteratively refined
  `docs/superpowers/specs/2026-08-08-ip-038-slice-4-config-bundle-flattening-design.md`
  through repeated independent spec review until approval.
- The final accepted spec now fixes the key Slice 4 contract seams:
  - single logical install → open-in-VS-Code entry with platform-specific
    wrappers;
  - equal `Try demo` / `Use local data` landing paths;
  - shipped read-only demo DB plus copied writable demo DB under
    `src/.dm-demo/sample_pm.db`;
  - `src/.env` / `DATABASE_PATH` mutation and preserve rules;
  - generated install-helper contract;
  - `bundle-manifest.json` provenance field
    `demo_db_source=generated_from_synthetic_loader`;
  - required runtime README onboarding handoff and bundled `.github` entry-surface
    checks.
- No Slice 4 implementation code has started yet. The next required gate is
  owner review of the written spec before implementation planning.
...
- The owner accepted the validated, independently reviewed Batch D result at
  local commits `a3eea27` (D1 deprecated operator-path retirement) and
  `f1ed955` (D2 final compatibility/read-artifact cleanup).
- Acceptance covers only the frozen Batch D scope: retired superseded retained-
  source operator paths, deletion of the A4-replaced current-state
  compatibility views, and correction of the stale `/api/project-snapshots`
  legacy marker plus the focused rehearsal/UAT/test continuity updates tied to
  those changes.
- `/api/hiref` legacy-result-projection status and the dormant
  `import-resource-portal` / `import-hiref-report` source rows remain deferred
  compatibility/audit surfaces rather than accepted cleanup targets. No D3,
  broader redesign, runtime redesign, connector redesign, skills redesign, or
  mutable staffing-write redesign is authorized by this decision.
- Commit / push status: the accepted D1 and D2 runtime changes remain committed
  locally; this working tree carries only the acceptance log update in
  `PROGRESS.md`. No push, merge, tag, release, deployment, connector access, or
  real-data action was performed.
- Exact next action: if further IP-036 work is desired, freeze and explicitly
  authorize a new bounded post-Batch-D slice first; do not infer a D3.

### 2026-08-06 — IP-036 Batch D2 final compatibility/read-artifact cleanup implemented locally

- Confirmed this session started from the required clean D1 commit
  `a3eea2762e29ee5494b7f8b5b278b8750f9ec6a7`; `git status --short` was empty
  before applying the D2 diff, and D3 remained explicitly unauthorized.
- Implemented only the frozen D2 slice:
  - removed the A4-replaced `v_member_load` / `v_project_team` compatibility
    views from `src/pm_agent/current_state_staffing/schema.py`, leaving the
    canonical `current_state_staffing_*` publication tables as the only retained
    current-state staffing storage/read authority inside the product runtime;
  - updated `src/pm_agent/dashboard/server.py` so `/api/project-snapshots` no
    longer advertises `legacy-result-projection` and instead declares the new
    non-legacy projection contract `project-snapshot-list-projection-v1`,
    matching the route's existing delegation to the canonical
    `project-snapshot-list` use case;
  - moved the focused continuity checks from the deleted views to canonical
    current-state publication tables in `src/tests/test_current_state_staffing.py`,
    `src/tests/test_data_integrity_concurrency.py`,
    `tools/rehearse_release.py`, `tools/tests/test_rehearse_release.py`, and
    `docs/REAL_ENVIRONMENT_UAT_RUNBOOK.md`;
  - updated `src/tests/test_unified_use_case_contract.py` so the dashboard
    project-snapshot projection now proves the new non-legacy contract instead
    of the stale legacy marker.
- Preserved the frozen D2 semantics explicitly:
  - `/api/hiref` remains an explicit `legacy-result-projection` surface because
    the retained B4 slot/result projection is still the serving artifact there;
  - dormant `import-resource-portal` / `import-hiref-report` source rows remain
    registered because historical sync-run/audit continuity still depends on
    them;
  - no D3 work, no broader redesign, no runtime redesign, no connector redesign,
    no skills redesign, and no mutable staffing-write redesign were introduced.
- Validation / review evidence:
  - `src/.venv/bin/python -m pytest src/tests/test_current_state_staffing.py src/tests/test_unified_use_case_contract.py src/tests/test_data_integrity_concurrency.py tools/tests/test_rehearse_release.py -q`
    (`44 passed`)
  - `src/.venv/bin/python -m ruff check src/pm_agent/dashboard/server.py src/pm_agent/current_state_staffing/schema.py src/tests/test_current_state_staffing.py src/tests/test_data_integrity_concurrency.py src/tests/test_unified_use_case_contract.py tools/rehearse_release.py tools/tests/test_rehearse_release.py`
  - `git diff --check`
  - independent read-only self-review: `D2 independent review passed`
- Commit / push status:
  - required clean local D1 commit `a3eea2762e29ee5494b7f8b5b278b8750f9ec6a7`
    remained at HEAD before applying this D2 diff;
  - the D2 changes are not committed and not pushed.
- Exact next recommended action:
  - stop for owner review or explicit commit/push instruction; do not enter D3.
...
- Implemented only the frozen C4 slice for retained source-family operator
  guidance:
  - updated `src/pm_agent/cli/commands/onboarding.py` help text so `pm
    onboarding` explicitly declares itself as the sole supported operator-visible
    entrypoint for retained structured sources and lists the retained source
    types on `profile save --help`;
  - demoted the surviving retained source-family operator scripts
    (`import_team_project_capacity_workbook.py`, the retained JSON import
    scripts, the retained registry CSV import scripts, `import_project_profiles.py`,
    and `import_cr_csv.py` via its owner parser) to explicit deprecated
    compatibility wrappers awaiting Batch D without changing their underlying
    owner behavior;
  - updated `README.md`, `src/README.md`,
    `docs/LOCAL_DATA_ONBOARDING_GUIDE.md`,
    `docs/EXTERNAL_IMPORT_FORMAT_MATRIX.md`, and
    `docs/REAL_ENVIRONMENT_UAT_RUNBOOK.md` so the supported operator guidance now
    points only to `pm onboarding` for retained workbook / JSON / registry /
    auxiliary file source families;
  - added focused `src/tests/test_data_onboarding.py` continuity coverage for
    onboarding help, wrapper help, and C4 doc wording.
- Preserved frozen C4 semantics explicitly:
  - `pm_agent.data_onboarding` remains only the orchestration/profile layer; no
    retained domain fact model or source-owner behavior moved into onboarding;
  - the wrapper scripts were not deleted and no runtime cleanup beyond wording
    demotion was introduced;
  - consumer-facing behavior, replay semantics, publication links, and retained
    capability ownership remain unchanged;
  - no Batch D cleanup, broader redesign, connector redesign, skills redesign,
    or mutable staffing-write redesign was folded into this working tree.
- Validation / review evidence:
  ...
  - independent read-only self-review against
    ...
    blocking C4 scope or acceptance defect; **C4 independent review passed**.
- Commit / push status:
  - required clean submitted baseline `695c8ec9d2153ceb2a684200ef647522ef1249e9`
    remained intact before this C4 diff was applied;
  - the C4 changes above are committed locally in this session and are not
    pushed.
- Exact next recommended action:
  - request owner decision on whether to authorize a separate Batch D session;
    do not enter Batch D, broader redesign, or cleanup/deletion work without
